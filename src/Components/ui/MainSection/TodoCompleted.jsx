  ...
  return ( <>
    ...
    <div className='flex flex-col flex-col-reverse w-full max-h-[90%]   gap-3  overflow-auto'>
          ...
          {completedTasks.map((items)=>{
            return(
              <div className='w-full text-mainTextclr min-h-16 bg-lightBgclr flex items-center py-4 px-6 rounded-md border-markclr border-2 font-semibold break-all' key={items.id}>{items.text}</div>
            )
          })}</div><div></div></>
  )
}
